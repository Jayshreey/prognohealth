                ...
                <div class="row">
                   <div class="col-md-8">
                      <div class="form-group text-left">
                         ...
                         <input class="form-control txt-submit" name="name" placeholder="{{ translate('Enter Name') }}" type="text" tabindex="1" value="{{ isset($review_category['name']) && $review_category['name'] != '' ? $review_category['name'] : ''  }}" required>
                      </div>
                   </div>
                @php
                      ...
                      <select name="is_active" class="form-control select2-modal" data-parsley-errors-container="#error_status" tabindex="2" required>
                         @if(isset($status_list) && !empty($status_list))
                            @foreach($status_list as $key => $value)
                               @php $selected = isset($review_category['is_active']) && $review_category['is_active'] == $key ? 'selected' : ''  @endphp
