@extends('backend.app')
@section('page_title'){{ translate('Certification List') }}@endsection
...
@section('content')
	<div class="page-header">
        <div>
            <h2 class="main-content-title tx-24 mg-b-5">{{ translate('Certification List') }}</h2>
            <ol class="breadcrumb">
		...
		<div class="btn btn-list">
			@if(valid_session('certification','add',false))
				<a class="btn ripple popup-page btn-primary" href="{{ route('admin.certification.new') }}"><i class="fe fe-plus-circle ml-2"></i> {{ translate('Add New') }}</a>
			@endif
		</div>
    </div>
	...
	<script>
		$(document).ready(function () {
			$('.data-list-table').DataTable({
				...
				"serverSide": true,
				"ajax": {
					"url": "{{ route('admin.certification.list') }}",
					"dataType": "json",
					"type": "POST",
					"data": {action : 'list'},
