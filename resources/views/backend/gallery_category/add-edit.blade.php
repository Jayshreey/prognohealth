                ...
                <div class="row">
                   <div class="col-md-8">
                      <div class="form-group text-left">
                         ...
                         <input class="form-control txt-submit" name="name" placeholder="{{ translate('Enter Name') }}" type="text" tabindex="1" value="{{ isset($gallery_category['name']) && $gallery_category['name'] != '' ? $gallery_category['name'] : ''  }}" required>
                      </div>
                   </div>
                @php
                      ...
                      <select name="is_active" class="form-control select2-modal" data-parsley-errors-container="#error_status" tabindex="2" required>
                         @if(isset($status_list) && !empty($status_list))
                            @foreach($status_list as $key => $value)
                               @php $selected = isset($gallery_category['is_active']) && $gallery_category['is_active'] == $key ? 'selected' : ''  @endphp
                     ...
                     <option value="0">{{ translate("No Parent") }}</option>
                        @if(isset($parent) && !empty($parent))
                           @foreach($parent as $key => $value)
                              @php $selected = isset($gallery_category['parent_id']) && $gallery_category['parent_id'] == $value->id ? 'selected' : ''  @endphp
                              <option value="{{ $value->id }}" {{ $selected }}>{{ get_gallery_category_path_name_string($value->path,$value->id) }}</option>
                           @endforeach
                        @endif
                     </select>
