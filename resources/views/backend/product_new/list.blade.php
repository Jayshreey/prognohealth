@extends('backend.app')
@section('page_title'){{ translate('Product List') }}@endsection
...
@section('content')
	<div class="page-header">
        <div>
            <h2 class="main-content-title tx-24 mg-b-5">{{ translate('Product List') }}</h2>
            <ol class="breadcrumb">
		...
		<div class="btn btn-list">
			<?php //if(empty($permissions) || isset($permissions['role']['edit'])){ ?>
				<a class="btn ripple btn-primary" href="{{ route('admin.product.new') }}"><i class="fe fe-plus-circle ml-2"></i> {{ translate('Add New') }}</a>
			<?php //} ?>
		</div>
    </div>
		...
		$(document).ready(function () {
			$('.data-list-table').DataTable({
				...
				"columns": [
					...
					{"data": "popular", orderable: false, className: 'text-center'},
