@extends('backend.app')
@section('page_title'){{ translate('Blog Category List') }}@endsection
...
@section('content')
	<div class="page-header">
        <div>
            <h2 class="main-content-title tx-24 mg-b-5">{{ translate('Blog Category List') }}</h2>
            <ol class="breadcrumb">
		...
		<div class="btn btn-list">
			<?php //if(empty($permissions) || isset($permissions['role']['edit'])){ ?>
				<a class="btn ripple popup-page btn-primary" href="{{ route('admin.blog_category.new') }}"><i class="fe fe-plus-circle ml-2"></i> {{ translate('Add New') }}</a>
			<?php //} ?>
		</div>
    </div>
	...
	<script>
		$(document).ready(function () {
			$('.data-list-table').DataTable({
				...
				"serverSide": true,
				"ajax": {
					"url": "{{ route('admin.blog_category.list') }}",
					"dataType": "json",
					"type": "POST",
					"data": {action : 'list'},
