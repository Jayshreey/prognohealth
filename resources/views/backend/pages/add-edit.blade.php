                        ...
                        <div class="col-md-12">
                            <div class="row">
                                <div class="col-md-8">
                                    <div class="form-group text-left">
                                        <label>{{ translate('Page Title') }} <span class="tx-danger">*</span></label>
                                        <input class="form-control txt-submit" name="title" placeholder="{{ translate('Enter Page Title') }}" type="text" tabindex="1" value="{{ isset($pages['title']) && $pages['title'] != '' ? $pages['title'] : ''  }}" required>
                                    </div>
                                </div>
                                    @php
                                    ...
                                    <div class="form-group text-left">
                                        ...
                                        <select name="is_active" class="form-control select2-modal" data-parsley-errors-container="#error_status" tabindex="2" required>
                                            @if(isset($status_list) && !empty($status_list))
                                                ...
                                                @php $selected = isset($pages['is_active']) && $pages['is_active'] == $key ? 'selected' : ''  @endphp
                            ...
                            <!-- <div class="row">
                                <div class="col-md-12 col-12">
                                    <div class="form-group text-left">
                                    <label>{{ translate('pages Category') }} <span class="tx-danger">*</span></label>
                                    <select name="pages_category_id" class="form-control select2-modal"  data-parsley-errors-container="#error_pages_category_id" required>
                                        <option value="">{{ translate("Select") }}</option>
                                        @if(isset($pages_category) && !empty($pages_category))
                                            @foreach($pages_category as $key => $value)
                                                @php $selected = isset($pages['pages_category_id']) && $pages['pages_category_id'] == $value->id ? 'selected' : ''  @endphp
                                                <option value="{{ $value->id }}" {{ $selected }}>{{ $value->name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    <span id="error_pages_category_id"></span>
                                    </div>
                                </div>
                            </div> -->
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <textarea class="form-control html_editor" id="descript" name="description" placeholder="{{ translate('description') }}" tabindex="4" required />{{ isset($pages['description']) && $pages['description']!='' ? $pages['description'] : '' }}</textarea>
                                    {{-- @php echo app_html_editor("description","description",translate('description'),isset($pages['description']) && $pages['description'] != '' ? $pages['description'] : '') @endphp --}}
                                    </div>
                                </div>
                            </div>
                        </div>
