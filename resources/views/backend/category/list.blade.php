@extends('backend.app')
@section('page_title'){{ translate('Category List') }}@endsection
...
@section('content')
	<div class="page-header">
        <div>
            <h2 class="main-content-title tx-24 mg-b-5">{{ translate('Category List') }}</h2>
            <ol class="breadcrumb">
		...
		<div class="btn btn-list">
			@if(valid_session('category','add',false))
				<a class="btn ripple popup-page btn-primary" href="{{ route('admin.category.new') }}"><i class="fe fe-plus-circle ml-2"></i> {{ translate('Add New') }}</a>
			@endif
		</div>
    </div>
		...
		$(document).ready(function () {
			$('.data-list-table').DataTable({
				...
				"columns": [
					...
					{"data": "country", searchable: false, className: 'text-center'},
