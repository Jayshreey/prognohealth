...
if ($ftp) {
   ...
   //echo dirname(__DIR__) . '/uploads/filemanager/source/';
   $path = realpath(dirname(__DIR__) . '/uploads/filemanager/source/');
 
if ($path && is_dir($path)) {
    $files = scandir($path. $rfm_subfolder .'/'. $subdir);
    // Process files
} else {
    ...
    if ($ftp) {
        ...
        if ($file != "." && $file != "..") {
            ...
            if (is_dir($config['current_path'] . $rfm_subfolder . $subdir . $file)) {
                ...
                $current_files_number++;
                 $file_path = rtrim($config['current_path'], '/\\') . '/' .
                    ltrim($rfm_subfolder, '/\\') .
                    ltrim($subdir, '/\\') .
                    ltrim($file, '/\\');
                ...
                if (file_exists($file_path) && is_file($file_path)) {
                    ...
                    $date = 0;
                    $size = 0;
                   // error_log("Missing or invalid file: " . $file_path);
                }
	...
	<div class="row-fluid ff-container">
        ...
        foreach ($files as $file_array) {
            ...
            foreach ($files as $nu=>$file_array) {
                ...
                $src_thumb=$config['base_url'].$src_thumb;
                if($mini_src==""){
                $is_icon_thumb_mini=false;
                }
